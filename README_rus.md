...
Режим запуска:
- сначала меняете параметры/чекбоксы;
- затем нажимаете `Run calculation` (или `Ctrl+Enter`);
- кнопка подсвечивает неприменённые изменения и состояние расчёта.
...
- Есть performance guard: слишком большой horizon отклоняется, чтобы UI не зависал на локальной машине.
...
Использование:
- после любого изменения параметров нажмите `Run calculation`;
- при некорректном диапазоне (`start > horizon`) кнопка запуска отключается до исправления.
